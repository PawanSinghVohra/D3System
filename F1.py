...
def index():
    ...
    directory = os.path.join("C:\\",
                             "Users\VOHRA\PycharmProjects\Analyse\input\SMNI_CMI_TRAIN\SMNI_CMI_TRAIN\co2a0000364")
    for root, dirs, files in os.walk(directory):
        for file in files:
            if file.endswith(".gz"):

                temp_df = pd.read_csv(
                    "C:\\Users\VOHRA\PycharmProjects\Analyse\input\SMNI_CMI_TRAIN\SMNI_CMI_TRAIN\co2a0000364\\" + file,
                    skiprows=4, nrows=256, delimiter=' ', usecols=[3])  ## read from the file to df
    ...
    directory = os.path.join("C:\\",
                             "Users\VOHRA\PycharmProjects\Analyse\input\SMNI_CMI_TEST\SMNI_CMI_TEST\co2c0000339")
    for root, dirs, files in os.walk(directory):
        for file in files:
            if file.endswith(".gz"):

                test_temp_df = pd.read_csv(
                    "C:\\Users\VOHRA\PycharmProjects\Analyse\input\SMNI_CMI_TEST\SMNI_CMI_TEST\co2c0000339\\" + file,
